...
export default function AnimalsTab() {
  ...
  const emptyAnimal = {
    ...
    registration_number: '', status: 'active', lactating: false, notes: '',
  }
  ...
  function openEdit(a) {
    setForm({
      ...
      status:a.status, lactating:a.lactating||false, notes:a.notes||'',
    })
            ...
            <div key={a.id} className="list-item" onClick={()=>{setSelId(a.id);setView('detail')}}>
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:'0.7rem', color:'var(--subtext)', fontFamily:'DM Mono, monospace' }}>
                  {a.breed} {age!=='—'&&`· ${age}`} {wt&&`· ${wt} lb`} {a.dam_tag&&`· dam ${a.dam_tag}`}
                </div>
              </div>
